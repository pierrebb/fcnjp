...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_BLESSE;
	$FrmAction="GesLocalisation_D.php";
	$FrmQuitter="Geslocalisation_L.php";
	$prg_Javascript='MAdm/JSGesLocalisation.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenLocalisation_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cLocalisation = new classLocalisation();
		$cLocalisation->InitClass($IdItem);
		$cLocalisation->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cLocalisation->GetCodeUnique($ActionPHP)){
			if ($cLocalisation->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cLocalisation->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
