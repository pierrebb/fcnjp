...
class classSituation{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_SituationSelectById;
			$PS = $Sql->ConstruireSQL("@IdSituation", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_SituationVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_SituationVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}

		$PS = $Sql->ConstruireSQL("@IdEquipe", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeBls", $this->Code, $PS);
	...
	function Delete(){
		...
		if ($this->Id > 0){
			$Sql= new ClassSQL();
			$PS=$ps_SituationDeleteById;
			$PS = $Sql->ConstruireSQL("@IdSituation", $this->Id, $PS);
	...
	function Validate($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_SituationInsert;
		break;
		case C_UPDATE:
			$PS=$ps_SituationUpdate;
		break;
		default:
			$PS = "";
		}
		...
		$PS = $Sql->ConstruireSQL("@IdSituation", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeSituation", strtoupper($this->Code), $PS);
