...
class classBlesse{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_BlesseSelectById;
			$PS = $Sql->ConstruireSQL("@IdBle", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_BlesseVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_BlesseVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}
	...
	function Delete(){
		...
		if ($this->Id > 0){
			$Sql= new ClassSQL();
			$PS=$ps_BlesseDeleteById;
			$PS = $Sql->ConstruireSQL("@IdBle", $this->Id, $PS);
	...
	function Validate($ActionPHP){
		...
		$PS = $Sql->ConstruireSQL("@IdBle", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeBls", strtoupper($this->Code), $PS);
