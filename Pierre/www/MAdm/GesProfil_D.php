...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_PROFIL;
	$FrmAction="GesProfil_D.php";
	$FrmQuitter="GesProfil_L.php";
	$prg_Javascript='MAdm/JSGesProfil.js';
	...
	if ($Bdd == 1){
		...
		$cProfil->InitClass($IdItem);
		$cProfil->PutCode($Code);
		$cProfil->PutActif($Actif);

		$Message = C_MES_SAVE_NOK;
		if ($cProfil->GetCodeUnique($ActionPHP)){
			if ($cProfil->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cProfil->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
...
<?
	if (($IdItem > 0) && ($ActionPHP==C_UPDATE) && ($Bdd==0) && ($MessageCode=="")){
		$cProfil= new classProfil();
		$cProfil->InitClass($IdItem);
		$Code=$cProfil->GetCode();
		$Actif=$cProfil->GetActif();
		$ActionPHP = C_UPDATE;
		unset($cMagasin);
	}
?>
