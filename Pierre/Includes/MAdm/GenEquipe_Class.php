...
class classEquipe{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_EquipeSelectById;
			$PS = $Sql->ConstruireSQL("@IdEquipe", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_EquipeVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_EquipeVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}
		...
		$PS = $Sql->ConstruireSQL("@CodeE", $this->Code, $PS);
	...
	function Validate($ActionPHP){
		...
		$PS = $Sql->ConstruireSQL("@IdEquipe", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeE", strtoupper($this->Code), $PS);
