...
class classInfoindivmatch{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_InfoindivmatchSelectById;
			$PS = $Sql->ConstruireSQL("@IdInfoindivmatch", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_InfoindivmatchVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_InfoindivmatchVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}

		$PS = $Sql->ConstruireSQL("@IdInfoIndivMatch", $this->Id, $PS);
	...
	function Delete(){
		...
		if ($this->Id > 0){
			$Sql= new ClassSQL();
			$PS=$ps_InfoindivmatchDeleteById;
			$PS = $Sql->ConstruireSQL("@IdInfoindivmatch", $this->Id, $PS);
	...
	function Validate($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_InfoindivmatchInsert;
		break;
		case C_UPDATE:
			$PS=$ps_InfoindivmatchUpdate;
		break;
		default:
			$PS = "";
		}
