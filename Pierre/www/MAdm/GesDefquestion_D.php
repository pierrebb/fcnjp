...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_DEFQUESTION;
	$FrmAction="GesDefquestion_D.php";
	$FrmQuitter="GesDefquestion_L.php";
	$prg_Javascript='MAdm/JSGesDefquestion.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenDefquestion_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cDefquestion = new classDefquestion();
		$cDefquestion->InitClass($IdItem);
		$cDefquestion->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cDefquestion->GetCodeUnique($ActionPHP)){
			if ($cDefquestion->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cDefquestion->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
