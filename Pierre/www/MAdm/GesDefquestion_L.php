...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_LISTE_DEFQUESTION;
	...
	if ($ActionPHP == C_DELETE && $IdItem != 0){
		...
		$Status = $cDefquestion->Delete();
		unset($cEquipe);
		if ($Status != 1) {
			header("location: ".C_SITE_DELETE_IMPOSSI."?Retour=".C_SITE_REPERTOIRE."www/MAdm/".$FrmQuitter);
		}
		$ActionPHP = "";
	}
?>
	...
	<?=get_TableDataHead("");?>
		...
		<tbody>
			<?
				...
				$Sql= new classSQL();
				$TabSql=$Sql->SelectSQL($ps_DefquestionSelectAll, C_LOG_NOLOG);
				unset($Sql);
				$NbL = count ($TabSql);
