...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_PARAMETREGPS;
	$FrmAction="GesParametreGPS_D.php";
	$FrmQuitter="GesParametreGPS_L.php";
	$prg_Javascript='MAdm/JSGesParametreGPS.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenParametreGPS_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cParametregps = new classParametregps();
		$cParametregps->InitClass($IdItem);
		$cParametregps->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cParametregps->GetCodeUnique($ActionPHP)){
			if ($cParametregps->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cParametregps->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
