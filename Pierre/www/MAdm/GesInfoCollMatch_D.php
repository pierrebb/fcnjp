...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_INFOCOLLMATCH;
	$FrmAction="GesInfoCollMatch_D.php";
	$FrmQuitter="GesInfoCollMatch_L.php";
	$prg_Javascript='MAdm/JSGesInfoCollMatch.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenInfoCollMatch_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cInfocollmatch = new classInfocollmatch();
		$cInfocollmatch->InitClass($IdItem);
		$cInfocollmatch->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cInfocollmatch->GetCodeUnique($ActionPHP)){
			if ($cInfocollmatch->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cInfocollmatch->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
