...
class classDefquestion{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_DefquestionSelectById;
			$PS = $Sql->ConstruireSQL("@IdQues", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_DefquestionVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_DefquestionVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}

		$PS = $Sql->ConstruireSQL("@IdQues", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeBQues", $this->Code, $PS);
	...
	function Validate($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_DefquestionInsert;
		break;
		case C_UPDATE:
			$PS=$ps_DefquestionUpdate;
		break;
		default:
			$PS = "";
		}
		...
		$PS = $Sql->ConstruireSQL("@IdQues", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeQues", strtoupper($this->Code), $PS);
