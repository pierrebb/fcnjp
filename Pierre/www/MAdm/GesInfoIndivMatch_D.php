...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_INFOINDIVMATCH;
	$FrmAction="GesInfoIndivMatch_D.php";
	$FrmQuitter="GesInfoIndivMatch_L.php";
	$prg_Javascript='MAdm/JSGesInfoIndivMatch.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenInfoIndivMatch_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cInfoindivmatch = new classInfoindivmatch();
		$cInfoindivmatch->InitClass($IdItem);
		$cInfoindivmatch->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cInfoindivmatch->GetCodeUnique($ActionPHP)){
			if ($cInfoindivmatch->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cInfoindivmatch->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
