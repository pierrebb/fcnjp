...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_STATUT;
	$FrmAction="GesStatut_D.php";
	$FrmQuitter="GesStatut_L.php";
	$prg_Javascript='MAdm/JSGesStatut.js';
	...
	if ($Bdd == 1){
		...
		$cStatut->InitClass($IdItem);
		$cStatut->PutCode($Code);
		$cStatut->PutActif($Actif);

		$Message = C_MES_SAVE_NOK;
		if ($cStatut->GetCodeUnique($ActionPHP)){
			if ($cStatut->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cStatut->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
...
<?
	if (($IdItem > 0) && ($ActionPHP==C_UPDATE) && ($Bdd==0) && ($MessageCode=="")){
		$cStatut= new classStatut();
		$cStatut->InitClass($IdItem);
		$Code=$cStatut->GetCode();
		$Actif=$cStatut->GetActif();
		$ActionPHP = C_UPDATE;
		unset($cMagasin);
	}
?>
