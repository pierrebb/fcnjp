...
class classLocalisation{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_LocalisationSelectById;
			$PS = $Sql->ConstruireSQL("@IdLoc", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_LocalisationVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_LocalisationVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}

		$PS = $Sql->ConstruireSQL("@IdLoc", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeLoc", $this->Code, $PS);
	...
	function Validate($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_LocalisationInsert;
		break;
		case C_UPDATE:
			$PS=$ps_LocalisationUpdate;
		break;
		default:
			$PS = "";
		}
		...
		$PS = $Sql->ConstruireSQL("@IdLoc", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeLoc", strtoupper($this->Code), $PS);
