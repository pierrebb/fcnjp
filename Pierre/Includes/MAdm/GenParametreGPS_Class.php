...
class classParametregps{
	...
	function InitClass($Id){
		...
		if ($Id > 0){
			$Sql= new ClassSQL();
			$PS = $ps_ParametregpsSelectById;
			$PS = $Sql->ConstruireSQL("@IdPargps", $Id, $PS);
	...
	function GetCodeUnique($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_ParametregpsVerifCodeInsert;
		break;
		case C_UPDATE:
			$PS=$ps_ParametregpsVerifCodeUpdate;
		break;
		default:
		$Status=0;
		}

		$PS = $Sql->ConstruireSQL("@IdPargps", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeGPS", $this->Code, $PS);
	...
	function Validate($ActionPHP){
		...
		{
		case C_INSERT:
			$PS=$ps_ParametregpsInsert;
		break;
		case C_UPDATE:
			$PS=$ps_ParametregpsUpdate;
		break;
		default:
			$PS = "";
		}
		...
		$PS = $Sql->ConstruireSQL("@IdPargps", $this->Id, $PS);
		$PS = $Sql->ConstruireSQL("@CodeGPS", strtoupper($this->Code), $PS);
