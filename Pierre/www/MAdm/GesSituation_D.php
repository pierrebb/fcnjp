...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_SITUATION;
	$FrmAction="GesSituation_D.php";
	$FrmQuitter="GesSituation_L.php";
	$prg_Javascript='MAdm/JSGesSituation.js';
	...
	require_once $_SERVER['DOCUMENT_ROOT'].C_SITE_REPERTOIRE."Includes/"."MAdm/GenSituation_Class.php";

	// MAJ
	if ($Bdd == 1){
	    $bOk=1;

		$cSituation = new classSituation();
		$cSituation->InitClass($IdItem);
		$cSituation->PutCode($Code);

		$Message = C_MES_SAVE_NOK;
		if ($cSituation->GetCodeUnique($ActionPHP)){
			if ($cSituation->Validate($ActionPHP)) {
				$Message = C_MES_SAVE_OK;
				$IdItem=$cSituation->GetId();
				$ActionPHP = C_UPDATE;
			}
		}
		else{
