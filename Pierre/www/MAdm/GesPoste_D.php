...
//<!-- Modif:	-->
	...
	$FrmTitre=C_TITRE_POSTE;
	$FrmAction="GesPoste_D.php";
	$FrmQuitter="GesPoste_L.php";
	$prg_Javascript='MAdm/JSGesPoste.js';
	...
	if ($Bdd == 1){
		...
		$cPoste->InitClass($IdItem);
		$cPoste->PutCode($Code);
		$cPoste->PutActif($Actif);

		$Message = C_MES_SAVE_NOK;
		if ($cPoste->GetCodeUnique($ActionPHP)){
			if ($cPoste->Validate($ActionPHP)) {
